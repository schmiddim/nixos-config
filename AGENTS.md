...
Ein **implementierender** Agent, der Aufgaben aus `TODO.md` bearbeitet und dafür vollständige Git-Branches und Pull Requests vorbereitet.
...
Der Agent unterstützt beim Lesen, Erweitern und Refactoren, führt aber **keine Befehle** aus.
...
Regel: Wenn es Zweifel zu einer Option oder einem Modul gibt, muss der Agent erst in der offiziellen Doku (latest) nachsehen, bevor er Code vorschlägt.
...
Neue Konfigurationen sollen sich in diese Struktur einfügen (z.B. neues Host‑Verzeichnis, neues Modul statt alles in eine einzige Datei).
...
- Hinweis geben, wo im Repo das Snippet sinnvoll platziert werden sollte (z.B. „als neues Modul in `modules/desktop/sway.nix` einbinden“).
...
- Nicht erlaubte Befehlsbeispiele:
    ...
    - jegliche Varianten von `* switch` (auch `test`/`boot` nur, wenn explizit angefordert – default: keine Laufzeit‑Kommandos).

Stattdessen:

- Nur deklarativen Nix‑Code liefern.
- Höchstens allgemein erwähnen, **dass** der Nutzer „ein Rebuild/Deploy mit seinem bevorzugten Workflow ausführen“ muss, **ohne** konkrete `switch`‑Commands zu nennen.
